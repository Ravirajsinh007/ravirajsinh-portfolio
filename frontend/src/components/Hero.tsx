...
export default function Hero({ onExploreClick }: { onExploreClick: () => void }) {
  ...
  useEffect(() => {
    fetch(getApiUrl("/api/profile"))
      ...
      .catch((err) => console.error("Error loading profile in Hero.tsx:", err));
  }, []);

  const handleResumeAction = () => {
    if (profile?.resumeUrl) {
      const url = profile.resumeUrl.startsWith("/") ? getApiUrl(profile.resumeUrl) : profile.resumeUrl;
      ...
      <div className="w-full max-w-4xl flex flex-col items-center text-center gap-6">
        ...
        >
          ...
          >
            ...
            <span>{profile?.resumeUrl ? "Download Resume (PDF)" : "Print / Save Resume"}</span>
          </button>
        </motion.div>
      </div>
